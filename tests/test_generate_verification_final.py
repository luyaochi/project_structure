...
from generate_verification import main


class TestGenerateVerificationFinal(unittest.TestCase):
    """驗證報告生成最終測試"""
        ...
        with patch.object(sys, 'argv', test_args):
            try:
                ...
                for lang in ['zh-TW', 'zh-CN', 'en']:
                    ...
                    verification_file = Path.cwd() / f"VERIFICATION{suffix}.md"
    ...
    def test_main_single_lang_without_output(self):
        ...
        with patch.object(sys, 'argv', test_args):
            try:
                main()
                # 應該生成預設名稱的文件
                verification_file = Path.cwd() / "VERIFICATION.zh-CN.md"
                if verification_file.exists():
                    verification_file.unlink()
            except SystemExit:
                pass
